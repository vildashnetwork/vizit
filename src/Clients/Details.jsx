...
import { BottomTabs, Container, Footer, Head, SideNav } from "../pages/LandingPage/LandingPage";
import { data } from "../data/listingdata";
import LocationOnIcon from "@mui/icons-material/LocationOn";
import dayjs from "dayjs";
import { cleanString } from "../utils/cleanString";
import "../pages/PropertyDetails/dStyles.css";
...
cleanString;
MapComponent;
function PropertyDetail({ propertyId, currentUser, OwnerId }) {
    ...
    useEffect(() => {
        const check = () => {
            const compare = OwnerId == currentUser._id
            if (compare) {
                setisAdmin(true)
            } else {
                setisAdmin(false)
    ...
    const REVIEW_KEY = (id) => `reviews_${id}`;

    function getStoredReviews(propertyId) {
        const raw = localStorage.getItem(REVIEW_KEY(propertyId));
        if (!raw) {
            return { overallRating: 0, totalReviews: 0, entries: [] };
        }
        try {
    ...
    function saveStoredReviews(propertyId, reviews) {
        localStorage.setItem(REVIEW_KEY(propertyId), JSON.stringify(reviews));
    }

    function recalcRatings(entries) {
        if (!entries.length) return { overallRating: 0, totalReviews: 0 };
        const sum = entries.reduce((a, b) => a + b.rating, 0);
        return {
            overallRating: Number((sum / entries.length).toFixed(1)),
            totalReviews: entries.length
        };
    }


    function StarInput({ value, onChange, size = "large" }) {
        return (
                ...
                {[1, 2, 3, 4, 5].map((n) =>
                    n <= value ? (
                        <StarIcon
                            key={n}
                            fontSize={size}
                            style={{ color: "#88661bff", cursor: "pointer" }}
                            onClick={() => onChange(n)}
                        />
                    ) : (
                        <StarOutlineOutlinedIco
                            key={n}
                            fontSize={size}
    ...
    const images = house.images?.length ? house.images : [house.image];
                ...
                {data[0].images.map((image, index) => {
                  ...
                  ></button>
                            ...
                            <div className="dots-container">
                                {images.map((_, index) => (
                                    <button
                                        key={index}
                                        className="dots"
                                        style={{ opacity: index === imageIndex ? 1 : 0.4 }}
                                        onClick={() => setImageIndex(index)}
                                    />
                                ))}
                            </div>
                    ...
                    <div className="description-card">
                        ...
                        <ReviewRatingSection propertyId={propertyId} currentUser={currentUser} isAdmin={isAdmin} />
                    </div>
                </div>
